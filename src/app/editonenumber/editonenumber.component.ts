...
export class EditonenumberComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.idnumber = this.authService.getEditNumber();
    if (this.idnumber === -1) {
      this.router.navigate(['/']);
    }
  ...
  loadBlanc() {
    this.lg.getGuideFeatures().subscribe( (value: any[]) => {
      ...
      this.listFeatures.forEach( (feature) => {
        this.addroomsForm.addControl('typefeature' + feature.id, new FormControl(''));
      });

      // после того как создали фичи наполняем их значениями
      this.ns.getNumberFeature(this.idnumber).subscribe( (featurenumber: any[]) => {
        console.log(featurenumber);
        featurenumber.forEach(( feature) => {
          const el = document.getElementById('customCheck' + feature.id_features) as HTMLInputElement;
          el.checked = true;
    ...
    this.lg.getGuideBedstype().subscribe( (value: any[]) => {
      ...
      // после того как создали кровати наполняем их значениями
      this.ns.getNumberBedstype(this.idnumber).subscribe( (bedsnumber: any[]) => {
        bedsnumber.forEach( (bed, index, add) => {
          this.addroomsForm.controls['bed' + bed.id_bedstype].setValue(bed.amount);
        });
